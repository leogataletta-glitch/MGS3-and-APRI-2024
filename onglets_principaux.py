...
def render(key, codes, selected, description=None):
    ...
    {root} [role="radiogroup"]>div:has(>label){{display:contents!important;}}
    {root} [role="radiogroup"] label>div{{display:flex!important;justify-content:center!important;align-items:center!important;width:100%!important;}}
    {root} [role="radiogroup"] label[data-testid="stRadioOption"]>div>div:first-child:not([data-testid="stMarkdownContainer"]){{display:none!important;}}
    {root} [role="radiogroup"] label [data-testid="stMarkdownContainer"]{{width:100%!important;text-align:center!important;}}
    {root} [role="radiogroup"] label p{{margin:0!important;}}
    ...
    {root} [role="radiogroup"] label:has(input:checked){{background:white!important;box-shadow:0 5px 18px rgba(25,55,42,.13)!important;border:1px solid #e0e7e2!important;border-radius:20px!important;}}
    {root} [role="radiogroup"] label:has(input:checked)::after{{display:none!important;content:"";position:absolute;bottom:7px;top:auto;left:calc(50% - 4px);right:auto;width:8px;height:8px;background:#176344;border-radius:50%;}}
