...
def render(cat, fixed_mode=None):
    ...
    st.subheader(_locale_text(t('Les associations les plus fortes', 'Strongest associations')))
    st.caption(_locale_text(t('Unité : section communale (10 au maximum). Classement exploratoire par |ρ de Spearman|, sans preuve de causalité ni test de significativité. Une association entre sections ne décrit pas nécessairement les ménages.',
    ...
    measure = st.selectbox(_locale_text(t('Mesure des indicateurs', 'Indicator measure')), ['valeurs', 'scores_corriges'], format_func=_locale_formatter(lambda x: t('Valeurs brutes', 'Raw values') if x == 'valeurs' else t('Scores de résilience', 'Resilience scores')), key='corr_measure')
    minimum_n = st.slider(_locale_text(t('Réponses valides minimum par section pour les variables', 'Minimum valid responses per section for survey variables')), 10, 100, 30, 10)
    path = M._trouver('resultats.json')
    if not path:
        st.info(_locale_text(t('Résultats indisponibles.', 'Results unavailable.')))
        return
    ...
    mode = fixed_mode or st.selectbox(_locale_text(t('Relations recherchées', 'Relationship types')), ['all', 'indicators', 'variables'], format_func=_locale_formatter(lambda x: {'all': t('Toutes', 'All'), 'indicators': t('Indicateur ↔ indicateur', 'Indicator ↔ indicator'), 'variables': t('Indicateur ↔ variable', 'Indicator ↔ variable')}[x]))
    focus = st.selectbox(_locale_text(t('Centrer sur un indicateur', 'Focus on an indicator')),
                        [None] + [k for k in frame if meta[k]['kind'] == 'indicator'],
                        format_func=_locale_formatter(lambda k: t('Tous', 'All') if k is None else meta[k]['label']))
    if not st.toggle(_locale_text(t('Afficher le classement', 'Show ranking')), key='corr_show'):
        return
    ...
    if ranked.empty:
        st.info(_locale_text(t('Pas assez de données comparables (au moins 8 sections).', 'Insufficient comparable data (at least 8 sections).')))
        return
    if mode != 'all':
        ranked = ranked[ranked.type == mode]
    if focus:
        ranked = ranked[(ranked.a == focus) | (ranked.b == focus)]
    st.caption(_locale_text(t(f'{len(ranked)} paires comparées. Les liens provenant de la même question sont exclus. Les données manquantes ne sont pas remplacées par zéro.',
                 f'{len(ranked)} pairs compared. Links from the same source question are excluded. Missing data are not replaced with zero.')))
    if ranked.empty:
        st.info(_locale_text(t('Aucune paire pour cette sélection.', 'No pairs for this selection.')))
        return
    ranked = ranked.copy()
    ranked['low'] = np.nan
    ranked['high'] = np.nan
    ...
    st.caption(_locale_text(t('Les liens parfaits sont signalés : ils peuvent provenir de la définition des mesures ou d’une question source non identifiée. Ils ne constituent pas automatiquement une découverte indépendante.',
    ...
    st.caption(_locale_text(t('Sensibilité calculée pour les 20 premières paires uniquement. ρ positif : les deux mesures augmentent ensemble ; négatif : elles évoluent en sens opposés. Les bornes −1 section montrent la sensibilité au retrait de chaque section, pas un intervalle de confiance. Parmi beaucoup de paires, de fortes corrélations peuvent apparaître par hasard. Des définitions proches peuvent aussi créer des liens mécaniques.',
    ...
    st.download_button(_locale_text(t('Télécharger le classement CSV', 'Download ranking CSV')), export.to_csv(index=False).encode('utf-8-sig'), 'correlations_sections.csv', 'text/csv')
    selected = st.selectbox(_locale_text(t('Examiner une association', 'Inspect an association')), list(ranked.head(20).index),
        format_func=_locale_formatter(lambda idx: meta[ranked.loc[idx, 'a']]['label'] + ' ↔ ' + meta[ranked.loc[idx, 'b']]['label']))
    ...
    st.download_button(_locale_text(t('Télécharger les points CSV', 'Download points CSV')), detail.to_csv().encode('utf-8-sig'), 'correlation_points.csv', 'text/csv')
    with st.expander(_locale_text(t('Sources et limites', 'Sources and limitations'))):
