...
async function exporterCarte(format){
  ...
  try{
    ...
    if(scale){const r=scale.getBoundingClientRect(),w=r.width;ctx.fillStyle='rgba(255,255,255,.94)';ctx.fillRect(12,height-49,w+24,39);ctx.strokeStyle='#243e34';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(22,height-29);ctx.lineTo(22,height-20);ctx.lineTo(22+w,height-20);ctx.lineTo(22+w,height-29);ctx.stroke();ctx.fillStyle='#243e34';ctx.font='12px Arial';ctx.save();ctx.textAlign='center';ctx.fillText(scale.textContent,22+w/2,height-36);ctx.restore();}
