...
def render(cat):
    ...
    if not cat:
        st.info(_locale_text(t('Les réponses individuelles ne sont pas disponibles.', 'Individual responses are unavailable.')))
        return
    import relations_guide
    relations_guide.render(fr)
    view = st.selectbox(_locale_text(t('Explorer les relations', 'Explore relationships')), ['manual', 'individual', 'indicators', 'mixed'], format_func=_locale_formatter(lambda k: {'manual': t('Comparaison détaillée', 'Detailed comparison'), 'individual': t('Variables ↔ variables', 'Variables ↔ variables'), 'indicators': t('Indicateurs ↔ indicateurs', 'Indicators ↔ indicators'), 'mixed': t('Indicateurs ↔ variables', 'Indicators ↔ variables')}[k]))
    if view == 'individual':
    ...
    st.caption(_locale_text(t('Comparez des réponses observées. Une association ne démontre pas un effet causal.',
                 'Compare observed answers. An association does not establish a causal effect.')))
    left, right = st.columns(2)
    with left:
        qid = st.selectbox(_locale_text(t('Variable à expliquer', 'Outcome variable')), list(lookup),
            index=list(lookup).index(rice), format_func=_locale_formatter(lambda k: L.libelle(lookup[k])), key='rel_y')
    ...
    with right:
        x = st.selectbox(_locale_text(t('Variable de comparaison','Comparison variable')), options,
            index=options.index('g:paysage'), format_func=_locale_formatter(lambda k: reg_names[k[2:]] if k.startswith('g:') else L.libelle(lookup[int(k[2:])])), key='rel_x')
    q = lookup[qid]
    masks, base = question_masks(cat, q)
    if not masks:
        st.info(_locale_text(t('Aucune réponse exploitable.', 'No usable answers.')))
        return
    target_name = st.selectbox(_locale_text(t('Réponse étudiée','Answer of interest')), list(masks),
        index=list(masks).index('Riz') if 'Riz' in masks else 0, key=f'rel_mod_{qid}', format_func=_locale_formatter(str))
    ...
    if agricult:
        farmers = st.toggle(_locale_text(t('Seulement les répondants pratiquant l’agriculture','Only respondents practising agriculture')),
    ...
    with st.expander(_locale_text(t('Restreindre à une réponse ou un sous-groupe','Restrict to an answer or subgroup'))):
        fqid = st.selectbox(_locale_text(t('Question de filtre','Filter question')), [None]+list(lookup),
                            format_func=_locale_formatter(lambda k: t('Aucun filtre','No filter') if k is None else L.libelle(lookup[k])), key='rel_fq')
        if fqid is not None:
            fm, _ = question_masks(cat, lookup[fqid])
            chosen = st.multiselect(_locale_text(t('Réponses retenues (au moins une)','Included answers (at least one)')), list(fm), key=f'rel_fm_{fqid}', format_func=_locale_formatter(str))
            if not chosen:
                st.info(_locale_text(t('Choisissez une réponse pour appliquer ce filtre.','Choose an answer to apply this filter.')))
                return
    ...
    if x.startswith('g:'):
        ...
        xq = lookup[int(x[2:])]
        if xq['i'] == qid:
            st.info(_locale_text(t('Choisissez deux variables différentes.','Choose two different variables.')))
            return
        ...
        if overlaps.any():
            if not xm:
                return
            xm_name = st.selectbox(_locale_text(t('Réponse de comparaison','Comparison answer')), list(xm),key=f'rel_xm_{xq["i"]}', format_func=_locale_formatter(str))
    ...
    st.caption(_locale_text(t(f'{counts["used"]} répondants analysés sur {counts["population"]} dans la population retenue. ',
    ...
    if rows.empty:
        st.info(_locale_text(t('Aucune observation comparable avec ces choix.','No comparable observations for these selections.')))
        return
    st.caption(_locale_text(t('Dénominateur : réponses valides à la question, dans chaque groupe. Non-réponse, refus et question non posée ne valent jamais « non ».',
    ...
    components.html(_locale_html('<div style="width:100%;overflow:auto">'+svg.replace('width="900"', 'style="width:100%;min-width:600px"')+'</div>'), height=115+64*len(rows), scrolling=True)
    if len(rows) == 2:
    ...
    if (rows.n < 30).any():
        st.info(_locale_text(t('Certains groupes ont moins de 30 répondants : leurs résultats sont fragiles.','Some groups have fewer than 30 respondents: interpret them cautiously.')))
    st.caption(_locale_text(t('Traits : intervalle exploratoire à 95 %, obtenu en rééchantillonnant les sections communales entières (2 000 tirages). Aucun intervalle si moins de 4 sections ou section inconnue. Avec seulement 10 sections dans l’enquête, ces intervalles restent approximatifs.',
    ...
    with st.expander(_locale_text(t('Méthode et tableau des effectifs','Method and counts'))):
        ...
        st.caption(_locale_text(t('Les effectifs ci-dessus sont recalculés ; les anciens effectifs dans les notes du questionnaire ne sont pas utilisés.',
